# Re-Fit AI Service

AI 기반 멘토-멘티 매칭 및 추천 서비스

---

## 📁 프로젝트 구조

```
19-team-project-ai/
├── ai_app/              # FastAPI 애플리케이션
│   ├── api/            # API 엔드포인트
│   ├── controllers/    # 비즈니스 로직
│   ├── services/       # 서비스 레이어
│   └── adapters/       # 외부 연동
├── .github/
│   └── workflows/
│       ├── ci.yml      # CI 파이프라인
│       ├── cd.yml      # CD 파이프라인
│       └── rollback.yml # 롤백 워크플로우
├── scripts/            # 유틸리티 스크립트
├── pyproject.toml      # Python 프로젝트 설정 (단일 소스)
└── requirements.txt    # 팀원 참조용 (자동 동기화)
```

---

## 🚀 배포 프로세스

### **CI/CD 파이프라인**

```
PR 생성
    ↓
[CI] Lint & Test (PR only)
    ↓
머지 to develop
    ↓
[CI] Integration Test
    ↓
[CI] Wheel 빌드 & S3 업로드
    ↓
[CD] 트리거 (자동)
    ↓
[CD] 배포 to EC2
    ↓
헬스체크 & Discord 알림
```

### **배포 단계 (CD)**

1. **백업 생성**
   - 코드 백업: `/backups/ai/code_YYYYMMDDHHMMSS/`
   - Wheel 백업: `/backups/ai/wheel_YYYYMMDDHHMMSS/`

2. **Wheel 패키지 설치**
   - S3에서 다운로드
   - `pip install --force-reinstall --no-cache-dir`

3. **서비스 재시작**
   - PM2 프로세스 정리
   - PM2 재시작
   - Caddy 리로드

4. **헬스체크**
   - 최대 5회 재시도
   - 실패 시 자동 롤백

---

## 🔄 롤백 시스템

### **롤백 워크플로우 사용법**

#### **1️⃣ 백업 목록 조회**

```bash
# GitHub Actions → AI Service Rollback 워크플로우 선택
# Mode: list 선택
```

**출력 예시:**
```
📂 Available Code Backups (Latest 10):
  - code_20260128100000 (Jan 28 10:00)
  - code_20260127150000 (Jan 27 15:00)
  ...

📦 Available Wheel Backups (Latest 10):
  - wheel_20260128100000 (Jan 28 10:00)
  - wheel_20260127150000 (Jan 27 15:00)
  ...
```

#### **2️⃣ 특정 버전으로 롤백**

```bash
# GitHub Actions → AI Service Rollback 워크플로우 선택
# Mode: restore
# Backup ID: 20260127150000  (또는 code_20260127150000)
# Restore dependencies: true
```

#### **3️⃣ 최신 백업으로 긴급 롤백**

```bash
# Backup ID를 비워두면 자동으로 최신 백업 사용
# Mode: restore
# Backup ID: (비움)
# Restore dependencies: true
```

### **롤백 옵션**

| 옵션 | 설명 | 추천 |
|------|------|------|
| **Restore dependencies: true** | 코드 + Wheel 패키지 모두 복원 | ✅ 추천 (완전 복구) |
| **Restore dependencies: false** | 코드만 복원, 현재 venv 유지 | ⚠️ 의존성 호환 확인 필요 |

### **롤백 프로세스**

```
롤백 시작
    ↓
Safety Backup 생성 (현재 상태 임시 백업)
    ↓
PM2 프로세스 완전 정리
    ↓
코드 복원 (ai_app, pyproject.toml, requirements.txt)
    ↓
의존성 복원 (선택적, wheel 재설치)
    ↓
PM2 재시작
    ↓
헬스체크 (최대 10회, 점진적 대기)
    ↓
성공 → Safety backup 삭제
실패 → Safety backup으로 자동 복구 시도
```

### **다단계 복구 전략**

1. **Level 1**: 지정된 백업으로 롤백
2. **Level 2**: 헬스체크 실패 시, Safety backup으로 복구
3. **Level 3**: 모든 시도 실패 시, 수동 개입 필요 (상세 로그 제공)

---

## 🔧 의존성 관리

### **pyproject.toml (단일 소스)**

- **CI/CD에서 사용**: `python -m build --wheel`
- **의존성 추가**: `[project].dependencies`에 직접 추가
- **예시**:
  ```toml
  dependencies = [
      "fastapi>=0.109.0",
      "sentence-transformers>=2.2.0",
      ...
  ]
  ```

### **requirements.txt (팀원 참조용)**

- **로컬 개발**: `pip install -r ai_app/requirements.txt`
- **자동 동기화**: CI에서 pyproject.toml과 자동 동기화
- **수동 업데이트 금지**: pyproject.toml에서만 수정

### **의존성 동기화**

```bash
# requirements.txt를 수정한 경우, CI가 자동으로 pyproject.toml 동기화
# PR에 자동 커밋됨
```

### **로컬에서 pyproject.toml 업데이트**

```bash
# 현재 설치된 패키지를 pyproject.toml에 반영
python scripts/freeze-to-pyproject.py
```

---

## 🛠️ 개발 가이드

### **로컬 개발 환경 설정**

```bash
# 1. 가상환경 생성
python3 -m venv venv
source venv/bin/activate

# 2. 의존성 설치
pip install -r ai_app/requirements.txt

# 3. 개발 의존성 설치
pip install -e ".[dev]"

# 4. 서비스 실행
cd ai_app
uvicorn api.main:app --reload --port 8000
```

### **의존성 추가**

```bash
# 1. pyproject.toml에 추가
[project]
dependencies = [
    "new-package>=1.0.0",
    ...
]

# 2. 로컬 설치
pip install new-package

# 3. requirements.txt 업데이트 (선택)
pip freeze | grep new-package >> ai_app/requirements.txt

# 4. 커밋 & 푸시 (CI가 자동 동기화)
```

---

## 🏥 헬스체크

### **엔드포인트**

```bash
# 기본 헬스체크
curl http://localhost:8000/api/ai/health

# 응답 (성공)
{"status": "ok"}

# 응답 (실패)
{"detail": "Not Found"}  # 또는 500 에러
```

### **서비스 상태 확인**

```bash
# PM2 상태
pm2 status ai-service

# 로그 확인
pm2 logs ai-service --lines 50

# 에러 로그
tail -f /home/ubuntu/refit/logs/ai/error.log
```

---

## 🚨 트러블슈팅

### **배포 실패**

1. **pip install 타임아웃**
   - SSM 타임아웃: 30분 (1800초)
   - 큰 패키지 (torch, spacy) 설치 시간 고려

2. **의존성 충돌**
   - `grpcio` 버전 고정: 1.60~1.70
   - `pyproject.toml`에서 버전 범위 조정

3. **포트 충돌 (Errno 98)**
   - CD가 자동으로 포트 8000 정리
   - PM2 프로세스 완전 정리 로직 포함

### **롤백 실패**

1. **백업 없음**
   - 최소 1회 성공 배포 필요
   - Safety backup으로 임시 복구

2. **의존성 불일치**
   - `Restore dependencies: true` 사용
   - Wheel 패키지 함께 복원

3. **헬스체크 실패**
   - PM2 로그 확인: `pm2 logs ai-service`
   - 서버 에러 로그: `/home/ubuntu/refit/logs/ai/error.log`

---

## 📊 백업 관리

### **자동 백업**

- **배포 시**: 코드 + Wheel 자동 백업
- **보관 기간**: 7일 (자동 삭제)
- **위치**: `/home/ubuntu/refit/backups/ai/`

### **백업 구조**

```
/home/ubuntu/refit/backups/ai/
├── code_20260128100000/      # 코드 백업
│   └── ai_app/
├── wheel_20260128100000/     # Wheel 백업
│   └── refit_ai_service-0.1.0-py3-none-any.whl
└── safety_before_rollback_*/  # 임시 백업 (롤백 시)
```

---

## 🔐 환경 변수

### **GitHub Secrets**

| Secret | 설명 |
|--------|------|
| `AWS_ACCESS_KEY_ID` | AWS 인증 |
| `AWS_SECRET_ACCESS_KEY` | AWS 인증 |
| `EC2_INSTANCE_ID` | 배포 대상 EC2 |
| `S3_ARTIFACTS_BUCKET` | Wheel 저장 S3 |
| `SERVER_BASE_PATH` | 서버 기본 경로 |
| `HEALTH_CHECK_URL` | 헬스체크 URL |
| `DISCORD_WEBHOOK` | 알림 웹훅 |

### **서버 환경변수 (.env)**

```bash
# /home/ubuntu/refit/app/ai/.env
GEMINI_API_KEY=your_api_key
DATABASE_URL=postgresql://...
```

---

## 🧪 테스트 가이드

### **CD 스모크 테스트**

배포 후 자동으로 실행되는 기본 성능 검증 (SLI/SLO 기반)

#### **테스트 항목**

| 항목 | API | SLO 목표 | 검증 방법 |
|------|-----|----------|-----------|
| **기본 헬스체크** | `GET /api/ai/health` | 200 OK | 최대 10회 재시도 |
| **추천 API** | `POST /api/ai/recommendations` | P95 < 3초 | 응답 시간 측정 |
| **응답 형식** | - | JSON 구조 | status 필드 확인 |

#### **테스트 플로우**

```
배포 완료
    ↓
서비스 초기화 대기 (10초)
    ↓
기본 헬스체크 (최대 10회)
    ↓
✅ 성공 → 스모크 테스트 시작
❌ 실패 → 자동 롤백
    ↓
추천 API 호출 & 응답 시간 측정
    ↓
응답 형식 검증
    ↓
✅ 모두 통과 → 배포 완료
⚠️  SLO 초과 → 경고 표시 (배포는 계속)
❌ 실패 → 자동 롤백
```

#### **스모크 테스트 결과 예시**

```bash
✅ 기본 헬스체크 성공! (HTTP 200)

============================================
SLO 스모크 테스트 시작
============================================

📊 Test 1/2: 추천 API 검증...
  ✅ 추천 API: 2.34초 (목표: < 3초, SLO 달성)

📊 Test 2/2: 헬스체크 상세 응답 검증...
  ✅ 헬스체크 응답 형식 정상

============================================
✅ 스모크 테스트 통과!
============================================
```

---

### **롤백 테스트 가이드**

롤백 워크플로우 동작을 검증하는 방법

#### **사전 준비**

1. **최소 1회 이상 성공한 배포 필요**
   - 백업 생성을 위해 필수
   - 없으면 롤백 불가

2. **백업 존재 확인**
   ```bash
   # 서버에서 확인
   ls -lt /home/ubuntu/refit/backups/ai/
   
   # 또는 GitHub Actions에서
   # Rollback 워크플로우 → Mode: list
   ```

#### **테스트 시나리오**

##### **Scenario 1: 백업 목록 조회**

```yaml
# GitHub Actions → AI Service Rollback
Mode: list
```

**예상 출력:**
```
📂 Available Code Backups (Latest 10):
  - code_20260128104517 (Jan 28 10:45)
  - code_20260128104029 (Jan 28 10:40)
  ...

📦 Available Wheel Backups (Latest 10):
  - wheel_20260128104517 (Jan 28 10:45)
  - wheel_20260128104029 (Jan 28 10:40)
  ...

💡 Usage:
  - Use the same timestamp for code and wheel
  - Format: 20260128104517
```

**검증:**
- ✅ 백업 목록이 표시됨
- ✅ code와 wheel이 동일한 timestamp로 매칭됨
- ✅ 최근 10개만 표시됨

---

##### **Scenario 2: 최신 백업으로 롤백 (기본)**

```yaml
# GitHub Actions → AI Service Rollback
Mode: restore
Backup ID: (비움)  # 자동으로 최신 백업 선택
Restore dependencies: true
```

**예상 플로우:**
```
백업 확인
    ↓
Safety Backup 생성 (현재 상태 임시 백업)
    ↓
PM2 프로세스 완전 정리
    ↓
코드 복원
    ↓
Wheel 재설치 (의존성 복원)
    ↓
PM2 재시작
    ↓
헬스체크 (최대 10회, 점진적 대기)
    ↓
✅ 성공 → Safety backup 삭제
❌ 실패 → Safety backup으로 자동 복구
```

**검증 항목:**
- ✅ Safety backup 생성됨
- ✅ 이전 코드로 복원됨
- ✅ 의존성도 복원됨 (wheel 재설치)
- ✅ PM2 프로세스 online
- ✅ 헬스체크 200 OK
- ✅ Discord 알림 발송
- ✅ Safety backup 자동 삭제

---

##### **Scenario 3: 특정 버전으로 롤백**

```yaml
Mode: restore
Backup ID: 20260127150000  # 특정 timestamp
Restore dependencies: true
```

**검증:**
- ✅ 지정한 timestamp의 백업 사용됨
- ✅ 코드와 wheel이 동일 timestamp에서 복원됨

---

##### **Scenario 4: 코드만 롤백 (의존성 제외)**

```yaml
Mode: restore
Backup ID: 20260128104517
Restore dependencies: false  ⚠️
```

**경고:**
- ⚠️ 현재 venv 유지 (의존성 변경 없음)
- ⚠️ 이전 코드 + 새 의존성 = 호환 문제 가능!

**사용 시점:**
- 의존성 변경이 없었던 배포만 롤백할 때
- 코드 버그만 수정할 때

**검증:**
- ✅ 코드만 복원됨
- ✅ pip list 확인 → 의존성 변경 없음

---

##### **Scenario 5: 롤백 실패 후 자동 복구**

**테스트 방법:**
1. 의도적으로 잘못된 backup_id 입력
2. 또는 백업 디렉토리 권한 변경

**예상 플로우:**
```
롤백 시도
    ↓
❌ 백업 복원 실패 (권한 오류 등)
    ↓
헬스체크 실패 (10회)
    ↓
Safety backup으로 자동 복구 시도
    ↓
✅ 원래 상태로 복구
또는
❌ 수동 개입 필요
```

**검증:**
- ✅ Safety backup이 자동으로 복구 시도
- ✅ PM2 로그 자동 출력
- ✅ 상세한 디버그 정보 표시
- ✅ Discord 롤백 실패 알림 (없음, 배포 실패만)

---

#### **롤백 테스트 체크리스트**

| 항목 | 테스트 방법 | 예상 결과 | 상태 |
|------|------------|-----------|------|
| **백업 목록 조회** | Mode: list | 백업 목록 표시 | ☐ |
| **최신 백업 롤백** | Backup ID 비움 | 자동 선택 & 성공 | ☐ |
| **특정 백업 롤백** | Backup ID 지정 | 지정 버전 복원 | ☐ |
| **의존성 복원** | restore_dependencies: true | wheel 재설치 | ☐ |
| **코드만 롤백** | restore_dependencies: false | venv 유지 | ☐ |
| **Safety backup** | - | 자동 생성 & 삭제 | ☐ |
| **자동 복구** | 롤백 실패 유도 | Safety backup 복구 | ☐ |
| **PM2 프로세스 정리** | - | 모든 ai-* 정리 | ☐ |
| **헬스체크** | - | 10회 재시도, 점진적 대기 | ☐ |
| **Discord 알림** | - | 롤백 성공/실패 알림 | ☐ |

---

#### **롤백 후 확인 사항**

```bash
# 1. PM2 상태 확인
pm2 status ai-service
# → online 상태 확인

# 2. 서비스 버전 확인
curl http://localhost:8000/api/ai/health
# → 응답 확인

# 3. PM2 로그 확인
pm2 logs ai-service --lines 20
# → 에러 없는지 확인

# 4. 백업 디렉토리 확인
ls -lt /home/ubuntu/refit/backups/ai/
# → Safety backup이 삭제되었는지 확인

# 5. 의존성 확인 (restore_dependencies: true인 경우)
source /home/ubuntu/refit/app/ai/venv/bin/activate
pip list | grep sentence-transformers
# → 이전 버전으로 복원되었는지 확인
```

---

#### **롤백 실패 시 수동 복구**

롤백이 완전히 실패한 경우 (드물지만 발생 가능)

```bash
# 1. Safety backup 확인
ls -lt /home/ubuntu/refit/backups/ai/safety_*

# 2. Safety backup으로 수동 복원
sudo cp -r /home/ubuntu/refit/backups/ai/safety_before_rollback_*/ai_app \
  /home/ubuntu/refit/app/ai/

# 3. 권한 복구
sudo chown -R ubuntu:ubuntu /home/ubuntu/refit/app/ai/

# 4. PM2 재시작
pm2 stop ai-service
pm2 delete ai-service
pm2 start /home/ubuntu/refit/infra/pm2/ecosystem.ai.config.js --only ai-service

# 5. 헬스체크
curl http://localhost:8000/api/ai/health
```

---

#### **롤백 테스트 주의사항**

1. **프로덕션 환경에서 테스트 금지**
   - 스테이징 환경에서만 테스트
   - 또는 트래픽이 적은 시간대 (새벽)

2. **백업 타임스탬프 확인**
   - code와 wheel의 timestamp가 동일해야 함
   - 다르면 의존성 불일치 가능

3. **Safety backup 용량**
   - 롤백마다 현재 상태 백업
   - 디스크 공간 확인 필요

4. **Discord 알림 확인**
   - 롤백 진행 중 알림 확인
   - 성공/실패 상태 확인

---

## 🔬 부하테스트 (향후 구현 예정)

### **Phase 2: Prometheus 메트릭 수집**

부하테스트와 실시간 모니터링을 위한 메트릭 인프라 구축

**구현 항목:**
1. FastAPI Prometheus 미들웨어
2. 요청/응답 메트릭 수집
3. P50, P95, P99 측정
4. /metrics 엔드포인트 노출

**참고 문서:**
- SLI/SLO 설계 문서 (Section 9.2)
- Prometheus Python Client

---

### **Phase 3: 부하테스트 설계**

SLO 목표 달성 여부를 검증하는 부하테스트

**테스트 시나리오:**
1. **추천 API 부하**
   - 100회 요청, 동시 5개
   - P95 < 3초 검증
   - 성공률 > 97% 검증

2. **문서 분석 부하**
   - 50회 요청, 동시 3개
   - P95 < 30초 검증
   - 성공률 > 93% 검증

3. **리포트 생성 부하**
   - 20회 요청, 동시 2개
   - P95 < 70초 검증
   - 성공률 > 93% 검증

**도구 선택:**
- `hey` (간단, CLI)
- `k6` (강력, JS)
- `locust` (Python, UI)

---

### **Phase 4: CD 메트릭 기반 검증**

Prometheus 메트릭을 활용한 자동화된 SLO 검증

```yaml
# CD 워크플로우
- name: Verify SLO Metrics
  run: |
    # 배포 후 메트릭 수집 대기 (5분)
    sleep 300
    
    # Prometheus에서 P95 조회
    P95=$(curl -s "http://prometheus:9090/api/v1/query?query=refit:ai_latency:p95:1m{endpoint='/api/ai/recommendations'}" | jq -r '.data.result[0].value[1]')
    
    # SLO 검증
    if (( $(echo "$P95 > 3.0" | bc -l) )); then
      echo "❌ SLO 미달성: P95=${P95}초"
      exit 1
    fi
    
    echo "✅ SLO 달성: P95=${P95}초"
```

---

## 📊 성능 모니터링 (향후)

### **Grafana 대시보드 (Phase 3 이후)**

**구성 예정:**
1. AI Performance Dashboard
2. AI Quality Dashboard
3. AI Cost Dashboard
4. Error Budget Dashboard

**메트릭 예시:**
- `refit:ai_availability:1m` - 가용성
- `refit:ai_latency:p95:1m` - P95 지연시간
- `refit:ai_completion_rate:1m` - 처리 성공률
- `refit:error_budget:burn_rate:28d` - Error Budget 소진율

---

## 📞 문의

- **팀**: Re-Fit DevOps
- **Repository**: https://github.com/100-hours-a-week/19-team-project-ai